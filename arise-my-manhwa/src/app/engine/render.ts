  ...
  try {
    if (renderingEngine === "OPENAI") {
      ...
      */
      ...
      if (!replicateApiModelVersion) {
        throw new Error(
          `invalid replicateApiModelVersion, you need to configure your REPLICATE_API_MODEL_VERSION in order to use the REPLICATE rendering engine`
        );
      }
    ...
    if (
      ...
      if (
        ...
        throw new Error(
          `invalid huggingfaceInferenceApiModelRefinerModel, you need to configure your RENDERING_HF_INFERENCE_API_REFINER_MODEL in order to use the INFERENCE_API rendering engine`
        );
      }
