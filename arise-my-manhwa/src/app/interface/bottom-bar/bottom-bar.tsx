...
function BottomBar() {
  ...
  const removeFromUpscaleQueue = useStore(
    (state) => state.removeFromUpscaleQueue
  );
    ...
    startTransition(() => {
      const fn = async () => {
        for (let [panelId, renderedScene] of Object.entries(upscaleQueue)) {
          try {
            console.log(
              `upscaling panel ${panelId} (${renderedScene.renderId})`
            );
            const result = await upscaleImage(renderedScene.assetUrl);
            await sleep(1000);
            if (result.assetUrl) {
              console.log(
                `upscale successful, removing ${panelId} (${renderedScene.renderId}) from upscale queue`
              );
              setRendered(panelId, {
                ...renderedScene,
  ...
  const hasFinishedGeneratingImages =
    allStatus.length > 0 &&
    allStatus.length - remainingImages === allStatus.length;
  useEffect(() => {
